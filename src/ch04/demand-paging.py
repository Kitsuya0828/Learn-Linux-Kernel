...
show_message("新規メモリ領域を獲得しました。Enterキーを押すと1秒に10MiBずつ、合計100MiBの新規メモリ領域にアクセスします: ")
input()

for i in range(0, ALLOC_SIZE, PAGE_SIZE):
    memregion[i] = 0
    if i % ACCESS_UNIT == 0 and i != 0:
        show_message(f"{i//(1024*1024)} MiBアクセスしました")
        time.sleep(1)

show_message("新規獲得したメモリ領域の全てにアクセスしました。Enterキーを押すと終了します: ")
input()
